  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div v-else-if="post">
      <div class="bg-white border border-gray-300 p-4 sm:p-6 mb-6">
        <div v-if="post.image_url" class="mb-4 sm:mb-6 flex justify-center bg-gray-100 rounded-lg overflow-hidden border">
           <img :src="post.image_url" alt="Gallery" class="w-full max-h-[60vh] sm:max-h-[500px] object-contain" />
        </div>
        <h1 class="text-xl sm:text-2xl font-normal text-gray-800 mb-2 break-words">{{ post.title }}</h1>
        <div class="text-sm text-gray-500 mb-6 pb-4 border-b break-words">
          작성자: {{ post.nickname }} | 작성일: {{ formatDate(post.created_at) }} | 조회수: {{ post.view_count }}
        </div>
        <div class="min-h-[100px] text-gray-800 leading-relaxed mb-8 whitespace-pre-wrap break-words"><p>{{ post.content }}</p></div>
        <div class="flex flex-col sm:flex-row sm:justify-between gap-3 border-t pt-4">
          <NuxtLink to="/gallery" class="px-4 py-2 border text-sm text-gray-600 hover:bg-gray-50 w-full sm:w-auto text-center">목록</NuxtLink>
          <ClientOnly>
            <button v-if="canDeletePost" @click="handleDeletePost" class="px-4 py-2 border text-sm text-red-600 hover:bg-red-50 w-full sm:w-auto">삭제</button>
          </ClientOnly>
        </div>
      </div>
